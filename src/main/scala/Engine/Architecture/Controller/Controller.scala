...
//          }
    ...
    case TellJoin1Actor =>
      ...
      var join1OpTag: OperatorTag = null
      principalBiMap.keySet().forEach(opTag => if(opTag.operator.contains("Join1")) {
        join1Principal = principalBiMap.get(opTag)
        join1OpTag = opTag
      })
      sender ! (join1Principal, join1OpTag)
    case Resume =>
    case msg => stash()
  }
