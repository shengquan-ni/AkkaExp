...
object Controller{
  ...
  private def jsonToOperatorMetadata(workflowTag: WorkflowTag, json:JsValue): OperatorMetadata ={
    ...
    json("operatorType").as[String] match{
      case "LocalScanSource" => new LocalFileScanMetadata(tag,Constants.defaultNumWorkers,json("tableName").as[String],json("delimiter").as[Char],null,null)
      case "HDFSScanSource" => new HDFSFileScanMetadata(tag,Constants.defaultNumWorkers,json("host").as[String],json("tableName").as[String],json("delimiter").as[Char],null,null)
